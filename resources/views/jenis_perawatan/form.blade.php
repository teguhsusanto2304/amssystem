...
@section('subtitle', 'Halaman Jenis Perawatan')
@section('content')
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                ...
                @endif
                        <div class="mt-4">
        @if(!is_null($data))
        {!! Form::model($data, ['method' => 'PATCH','route' => ['jenisperawatans.update', $data->id]]) !!}
        @else
        {!! Form::open(array('route' => 'jenisperawatans.store','method'=>'POST')) !!}
        @endif
        <div class="mb-3">
            <label class="form-label" for="formrow-firstname-input">Jenis Kunjungan</label>
            {!! Form::text('jenis_perawatan', (!is_null($data)?$data->jenis_perawatan:null), array('placeholder' => 'Jenis Kunjungan','class' => 'form-control')) !!}
        </div>
        ...
        <div class="mt-4">
            ...
            <a href="{{ route('jenisperawatans.index') }}" type="reset" class="btn btn-danger w-md">
                Cancel
            </a>
        </div>
    </div> <!-- end col -->
