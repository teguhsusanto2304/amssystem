<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateGroupLayanansTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('group_layanans', function (Blueprint $table) {
            $table->id();
            $table->string('group_layanan',100);
            $table->unsignedSmallInteger('data_status')->default(1);
            $table->unsignedBigInteger('service_unit_id');
            $table->timestamps();
            $table->foreign('service_unit_id')->references('id')->on('service_units')->onDelete('cascade'); 
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('group_layanans');
    }
}
